...
const HomePage = () => {
  ...
  const fetchVideoName = () => {
    fetch(" http://test-alb-668998651.ap-south-1.elb.amazonaws.com:80/api/auth/all-video-name")
      .then((res) => res.json())
      .then((data) => {
              ...
              >
                <div className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 w-[335px]">
                  <div className="h-[300px] w-full overflow-hidden cursor-pointer">
                    ...
                    >
                      <source
                        src={`http://test-alb-668998651.ap-south-1.elb.amazonaws.com:80/${videoData.video}`}
                        type="video/mp4"
                      />
                    </video>
                  </div>
